...
add_action( 'rest_api_init', 'nb_register_routes' );
   ...
   */
function nb_register_routes() {
    $version = '1';
    $namespace = 'artists_followed/v' . $version;
    $base = 'lists';
    register_rest_route( $namespace, '/' . $base, array(
        'methods'         => WP_REST_Server::READABLE,
        'callback'        => 'get_list_artists_followed',
...
// DEBUG
//    if ($nb_user_followed_artists_ids_meta[0])
//        $collection[] = $nb_user_followed_artists_ids_meta[0]->ID;
//TEST
    return rest_ensure_response( $nb_user_followed_artists_ids_meta );
//    return rest_ensure_response( $nb__artists_id );
...
//    return rest_ensure_response( $collection );
  ...
  */
   ...
   */
  public function nb_get_items_permissions_check( $request ) {
//TODO
    //return true; <--use to make readable by all
   //return current_user_can( 'edit_something' );
   /* if ( ! current_user_can( 'edit_posts' ) ) {
    return new WP_Error( 'rest_forbidden', esc_html__( 'Sorry, you cannot view this post resource.', 'my-text-domain' ), array( 'status' => 401 ) );
   ...
   * @return WP_Error|bool
   */
  public function nb_get_item_permissions_check( $request ) {
   ...
   * @return WP_Error|bool
   */
  public function nb_create_item_permissions_check( $request ) {
    return current_user_can( 'edit_something' );
  }
   ...
   * @return WP_Error|bool
   */
  public function nb_update_item_permissions_check( $request ) {
   ...
   * @return WP_Error|bool
   */
  public function nb_delete_item_permissions_check( $request ) {
    return $this->create_item_permissions_check( $request );
  }
...
// should call public WP_REST_Controller::filter_response_by_context( $data, $context )
   ...
   */
/*
  public function nb_get_collection_params() {
    return array(
      'page'     => array(
